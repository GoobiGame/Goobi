...
export class Platform {
  ...
  update(player, canvasWidth, delta) {
    ...
    if (this.isMoving) {
      let newX = this.x + this.speed * this.direction * delta;
    ...
    const playerOnPlatform =
      ...
      // Relaxed condition to ensure dropping triggers reliably
      pBottom >= this.y &&
      pBottom <= this.y + 10; // Allow a slightly larger range for detection
